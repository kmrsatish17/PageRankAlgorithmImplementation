...
public class PageRankConverge extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {
		...
		int res = ToolRunner.run(new PageRankConverge(), args);
		System.exit(res);
	...
	public int run(String[] args) throws Exception {
		...
		// Job to calculate the final page rank by iterating for auto convergence
		...
		for ( ;  ; ) {
			...
			if (Converge == true){
				...
				System.out.println("Iteration converged at loop number " + i);
				break;
			}
		...
		public void reduce(Text title, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			if (!titleString.contains("$$$$")) {
				...
				if (titleStringTr.equals(tempKey)) {
					...
					double previousPageRank = Double.parseDouble(df.format(Double.parseDouble(arrValue[1])));
					double newPageRank = Double.parseDouble(df.format(pageRank));
					
					System.out.println("%%%% previousPageRank " + previousPageRank);
					System.out.println("%%%% newPageRank " + newPageRank);
					
					if((!(previousPageRank==newPageRank)) && (Converge == true)){
